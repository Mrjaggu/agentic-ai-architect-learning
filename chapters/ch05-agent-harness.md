...
There's a simpler way to say all seven components at once, useful when you're explaining this to someone who doesn't want the diagram: separate the **brain** (the model — reasoning only), the **hands** (the sandbox and tools that actually touch the world), and the **history** (the append-only record of what happened). None of the three should double as another — the reasoning engine should not also be the filesystem, the permission system, and the audit log. The payoff is resilience along three independent axes: if the sandbox crashes, the history survives and a new run can resume from it; if you swap the model for a newer one next quarter, the tools and policy remain exactly as inspectable as they were; if a task must resume after an interruption, a fresh session reconstructs its state from artifacts and traces instead of a lossy retelling of the conversation. Anthropic's own Claude Managed Agents platform (public beta, April 2026) productizes exactly this split — session, harness, and sandbox as three separately swappable pieces — which is the strongest public evidence that the separation isn't an academic nicety; it's what a lab bets its own managed-agent product on.
...
- **Bound the request before Layer 1 even runs.** Before the harness enforces any policy, turn the incoming request into an explicit, typed object: what's the goal, what inputs does it have, what does a valid output look like, what constraints apply, and — critically — what specific, checkable conditions mean *done*. For the dispute-investigation agent this is barely more than Ch4's initial state: goal (resolve the dispute for this transaction), inputs (the transaction record, the customer profile), constraints (no tool outside the policy file), done-when (a decision is recorded, evidence is attached, a human is notified if the outcome is `hitl`). The payoff is narrow but real: a model can drift, rationalize, or quietly redefine what "done" means over a long run (Ch4's `check_policy` loop, for instance) — a written, non-negotiable done-when clause is what a human or an eval harness (Ch17) checks the final output against, instead of trusting the model's own claim that it finished.
...
- **The mature position, made concrete.** From §5: a thin harness from day one, thickened as traces reveal need — but "thin" and "thick" are vague enough to argue about in a design review, so give them levels instead. **Level 0** is a prompt and a model, nothing else — fine for a one-off, low-risk, human-reviewed task with no side effects. **Level 1** adds a context map and a tool policy — the agent knows where to look and what it may touch, but still has no bounds and no persistence. **Level 2** adds structured state, verification, and a bounded loop — this is where §3's Layers 1, 2, 3, and 5 land; the run can resume, and it stops instead of spiraling. **Level 3** adds permission separation, full tracing, and human-approval gates — this is §3's complete seven layers, reserved for anything that can issue a refund, block a card, or message a customer. Move up a level only when the task's blast radius earns the added complexity: a summarization tool with no side effects has no business at Level 3, and `block_card` has no business staying at Level 0.
...
### A case study worth reading end to end: OpenAI's Codex, building itself

In February 2026, OpenAI's own engineering team (Ryan Lopopolo) published an account of an internal product built almost entirely by their Codex coding agent — not a demo, a production internal tool with real usage. It's one of the few public accounts of harness engineering written by the people who did it, with real numbers attached, and it maps cleanly onto every component in this chapter's own vocabulary. Worth reading closely for that reason.

**The headline numbers.** Zero lines of the product's code were manually written. After five months the codebase reached roughly 1 million lines. Three engineers opened and merged roughly 1,500 pull requests at an average of 3.5 PRs per engineer per day — and throughput *increased*, not decreased, as the team grew to seven engineers, the opposite of the coordination-overhead curve a growing human engineering team usually rides. Individual Codex runs sometimes ran upwards of six hours unattended. The product now has hundreds of internal users, some of them daily power users.

**How their harness maps to this chapter's seven layers:**

- **Context assembly (§1, Ch8).** A short `AGENTS.md` — roughly 100 lines — is injected into every run, but it isn't the knowledge base itself; it's a *map*, with pointers into a structured `docs/` directory (`ARCHITECTURE.md`, `design-docs/`, `exec-plans/`, `PRODUCT_SENSE.md`, `QUALITY_SCORE.md`, `RELIABILITY.md`, `SECURITY.md`) that the agent pulls from as needed. This is Ch8's selection discipline — retrieve only what this step needs — applied at the scale of an entire engineering org's tribal knowledge, externalized into files instead of living in senior engineers' heads.
- **Tool policy, generalized into architecture.** Their rule: "enforce invariants, not implementations." Rather than reviewing every generated file for style, they fix a layering per domain — Types → Config → Repo → Service → Runtime → UI, with a single Providers interface for cross-cutting concerns like auth and telemetry — and enforce it mechanically with custom lints. The detail worth stealing: the lint error messages are written to double as *remediation instructions*, so a failed check doesn't just block the agent, it tells the agent exactly what to do next. That's policy-as-data (§5b) taken one step further — the policy doesn't only gate, it teaches.
- **Eval & observability hooks (Ch16), closed back into the harness.** The agent can drive the Chrome DevTools Protocol directly for DOM snapshots and screenshots, and query a Vector-fed observability stack (Victoria Logs/Metrics/Traces) with LogQL, PromQL, and TraceQL — so a prompt like "make sure no span in these four critical user journeys exceeds two seconds" is something the agent can go verify itself, not just something a human checks on a dashboard afterward. Observability stops being a human-only surface and becomes a tool the agent calls.
- **Bounds and merge gates (§3 Layer 3, §5b), deliberately minimized.** Their PRs are short-lived and merge gates are intentionally thin; a flaky test is usually handled with a re-run rather than a block. Their own stated reasoning: "in a system where agent throughput far exceeds human attention, corrections are cheap, and waiting is expensive" — and they explicitly flag that this would be irresponsible in a low-throughput environment. That caveat is the whole ballgame for what banking should and shouldn't borrow from this case study — see below.
- **Technical-debt cleanup, itself agentized.** They tried having humans manually chase architectural drift and it didn't scale — the same accretion pattern §2 describes, just for tech debt instead of incident patches. The fix: background Codex runs that scan for deviations from encoded "golden principles," update a quality grade, and open small, targeted refactor PRs — most reviewed in under a minute and auto-merged. The garbage-collection loop is a harness component in its own right, not a quarterly human cleanup sprint.
- **Human-judgment encoding (§5's "policy as data," extended to taste).** Review comments, refactor PRs, and user-reported bugs get captured as documentation updates — or, when documentation isn't enough to prevent recurrence, promoted directly into a lint or a code-level check. Their phrase for it: "human taste is captured once, then enforced continuously on every line of code." That is precisely this chapter's fail-closed, policy-as-data argument, generalized past risk controls into code quality and product taste.
- **End-to-end autonomy, with escalation as the explicit exception.** Given a single prompt, their agent validates current codebase state, reproduces a reported bug and records a video of it, implements and validates a fix by driving the running application, records a resolution video, opens the pull request, responds to review feedback, detects and remediates its own build failures, and merges — escalating to a human *only* when judgment is genuinely required. Escalation is the named exception path, not an afterthought; every non-judgment step sits inside the harness's bounds by design — this chapter's §5 "fail closed, escalate at the boundary" pattern taken to its logical end.

**The line worth remembering.** Their own framing of the job, stated almost exactly the way this chapter states it: *"what capability is missing, and how do we make it both legible and enforceable for the agent?"* — and the summary principle underneath everything above: *"Humans steer. Agents execute."* That's harness engineering as a job description, from a team operating at a scale most of us aren't yet at, but building the identical layer this chapter has been arguing for since §1.

**What a bank should and shouldn't borrow from this.** Everything above — invariants over implementations, remediation instructions baked into lint errors, taste captured once and enforced continuously, an agentized garbage-collection loop — transfers directly to a bank's *internal engineering surface*: test suites, internal tooling, CI configuration, dashboards, documentation. Borrow all of it there. What does **not** transfer is the minimal-blocking-gates choice itself, and OpenAI says so in their own writeup: it's the right trade only "in a system where agent throughput far exceeds human attention, corrections are cheap, and waiting is expensive." A reverted PR in an internal tool costs a re-run. A duplicate refund (Ch6 §2) or a wrongly blocked card costs a support call, a trust hit, and potentially a regulatory finding — that correction is not cheap, so the trade doesn't hold. Read correctly, the gate-placement rule this case study actually teaches isn't "fewer gates because autonomy scales" — it's "place gates by reversibility and blast radius, and let evidence tell you where they can safely thin out." Inside a bank, that means OpenAI's exact discipline for the reversible slice of the codebase, and Ch3's HITL placement — unchanged — for anything that touches money, a card, or a customer-facing message.

### A second case study: Anthropic's own harness work

Two more real, dated, verifiable data points worth having on hand, both from Anthropic itself rather than a third party studying it from outside.

**Harness design for long-running application development** (Anthropic Engineering, Prithvi Rajasekaran, March 2026) is a direct study of this chapter's Layer 2 argument — don't let the model grade its own work — run at the scale of multi-hour coding tasks. The team compared a single-agent "solo harness" against a three-agent harness (a planner that expands the spec, a generator that implements in sprints, and a separate evaluator that tests the result via browser automation) on the same task, a retro-game-maker build. The solo harness finished in 20 minutes for $9; the full three-agent harness took 6 hours and cost $200 — and produced a materially more complete, more correct result, because, in their own words, "separating the agent doing the work from the agent judging it proves to be a strong lever." That's Layer 2 generalized one level up: instead of a human validating one tool call's arguments, a *second agent* validates an entire deliverable — the same principle, applied where a human reviewing every intermediate step would be too slow to be practical. Their guiding design rule is worth pinning next to §6's Level 0-3 ladder: *"find the simplest solution possible, and only increase complexity when needed."*

**Claude Managed Agents**, launched in public beta in April 2026, is Anthropic's own answer to the build-vs-buy question §5 and §9 raise for the harness layer itself — a managed session/harness/sandbox split (the same brain/hands/history separation from §1) a team can adopt instead of building from scratch, sitting alongside AWS Bedrock AgentCore (above) and whatever an internal platform team builds in-house. A bank evaluating this decision now has three concrete reference points — roll-your-own (OpenAI's Codex writeup), one hyperscaler's managed offering (AWS Bedrock AgentCore), and the model vendor's own managed offering (Claude Managed Agents) — and the evaluation question is the same one Chapter 4 asked of orchestration frameworks: how much of §3's seven layers does the managed option actually own, and how much do you still have to build around it?
...
- For every blocking gate we've made thin or removed, can we point to the reversibility/blast-radius analysis that justified it — or did it just get thinner because it was slow?

A harness whose policy can be changed by the same engineer who writes the agent, without review, is a control on paper only — separation of duties applies to configuration, not just money. §7's OpenAI case study is a useful stress test for that last question: their "corrections are cheap" reasoning is legible and defensible for internal tooling; a bank that thins a gate on speed grounds alone, without the same explicit reasoning on record, has weakened a control instead of tuning one.
...
- "OpenAI's own Codex team put the harness engineer's job in one sentence: 'what capability is missing, and how do we make it both legible and enforceable for the agent?' That's the question I ask before every new tool grant."
- "Minimal blocking gates are the right trade only where corrections are cheap — OpenAI says so themselves about their own internal tooling. In banking, a duplicate refund isn't a cheap correction, so the gate stays."
- "Brain, hands, history: the model reasons, the sandbox acts, the trace remembers — and none of the three should double as another."
- "Anthropic's own harness research found what this chapter argues in Layer 2 at ten times the scale: a solo agent finished a task in 20 minutes; a three-agent harness with a separate evaluator took six hours and cost more — and was materially more correct. The accuracy was worth buying."
- "A run that was never bound to a written 'done when' clause wasn't given a task — it was given a vibe, and vibes don't hold up in an incident review."
